...
\tfdescription{G0}{$\INDCPA_\Enc^\Adversary()$: The IND-CPA game. The LR oracle encrypts using the actual PRF.}
\tfdescription{G1}{Replace $\mathrm{PRF}(k, r)$ with a truly random function $\RF(r)$.}
\tfdescription{Red1}{Reduction against PRF security, simulating Games \tfgamename{G0} and \tfgamename{G1} via an external PRF challenger.}
\tfdescription{G2}{Replace $\RF(r)$ with independently sampled randomness.}
\tfdescription{G3}{Ciphertext is uniformly random, independent of $m_b$.}
...
\tfcommentary{G3}{commentary/G3.tex}

\tffigure[Games $\tfgamename{G0}$--$\tfgamename{G3}$, Reduction $\tfgamename{Red1}$]{all_games}{G0,G1,G2,G3,Red1}
...
\input{commentary/Red1.tex}

\begin{figure}[ht]
\centering
\tfrendergame{indcpa}{Red1}
