...
\caption{Consolidated figure showing games $\tfgamename{G0}$--$\tfgamename{G2}$ and Reduction $\tfgamename{Red1}$.}
\label{fig:all-games}
\end{figure}
...
\centering
\input{G0.tex}
\caption{Game~$\tfgamename{G0}$: $\INDCPA_\Enc^\Adversary.\mathsf{Real}()$: the real IND-CPA game. The LR oracle encrypts using the actual PRF.}
\label{fig:G0}
\end{figure}
...
\centering
\input{G1.tex}
\caption{Game~$\tfgamename{G1}$: Replace $\mathrm{PRF}(k, r)$ with a truly random value.}
\label{fig:G1}
\end{figure}
...
\centering
\input{G2.tex}
\caption{Game~$\tfgamename{G2}$: $\INDCPA_\Enc^\Adversary.\mathsf{Ideal}()$: the ideal IND-CPA game. Ciphertext is uniformly random, independent of~$m_b$.}
\label{fig:G2}
\end{figure}
